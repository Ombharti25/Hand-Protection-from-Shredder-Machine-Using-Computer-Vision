...
def drawboxtosafeline(image_np,p1,p2,Line_Position2,Orientation):
    ...
    if (distance_from_line <= 0) :
			 ...
			 #sound = os.path.join()
             playsound(r"C:\Users\Om\PycharmProjects\Shreddersystem\utils\alert.wav")
             cv2.rectangle(image_np, (posii-20,20), (posii+85,60), (255,0,0), thickness=3, lineType=8, shift=0)
             #to write into xl-sheet

             led_on_url = "http://192.168.223.182:5000/ledoff"
             response = requests.get(led_on_url)
        ...
        led_off_url = "http://192.168.223.182:5000/ledon"
        response = requests.get(led_off_url)
